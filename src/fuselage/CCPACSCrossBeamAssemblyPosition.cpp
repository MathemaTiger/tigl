...
void CCPACSCrossBeamAssemblyPosition::BuildGeometry(bool just1DElements)
{
    ...
    TopExp::MapShapes(frame.GetGeometry(true, TiglCoordinateSystem::FUSELAGE_COORDINATE_SYSTEM), TopAbs_EDGE, edgeMap);
    BRepBuilderAPI_MakeWire wireBuilder;
    for (int i = 1; i <= edgeMap.Extent(); i++) //add the pices of edges of the frame to one frame
        wireBuilder.Add(TopoDS::Edge(edgeMap(i)));
    TopoDS_Face frameFace = BRepBuilderAPI_MakeFace(CloseWire(wireBuilder.Wire()));

    //get intersection between frame and the face
    TopoDS_Shape cutShape = CutShapes(GetCutGeometry(TiglCoordinateSystem::FUSELAGE_COORDINATE_SYSTEM), frameFace);

    if (just1DElements)
        m_geometry[just1DElements] = cutShape;
    else {
        ...
        TopTools_IndexedMapOfShape edgeMap;
        TopExp::MapShapes(cutShape, TopAbs_EDGE, edgeMap);
